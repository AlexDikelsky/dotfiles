
public class comissionedEmployee extends Employee {

	private double sales;
	
	public comissionedEmployee( String name, int id, String ssn, String address, int age, String departmentWorkIn,
					String responsibilities, double salary, double workHours)
	{
		super(name, id, ssn, address, age, departmentWorkIn, responsibilities, salary);

		this.sales = workHours;
	}
			
	public double totalSalary() {
		return this.sales * 10 + super.getSal();
	}
	
}
